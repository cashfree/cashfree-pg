...
> UploadVendorDocumentsResponse PGESUploadVendorsDocs(ctx, vendorId).XApiVersion(xApiVersion).DocType(docType).DocValue(docValue).File(file).Execute()
...
func main() {
    ...
    resp, r, err := cashfree.PGESUploadVendorsDocs(&xApiVersion, &vendorId, &docType, &docValue, &file, nil)
    if err != nil {
