...
> VendorRecon200Response PGESReconVendor(ctx).XApiVersion(xApiVersion).VendorReconRequest(vendorReconRequest).XRequestId(xRequestId).XIdempotencyKey(xIdempotencyKey).ContentType(contentType).Execute()
...
func main() {
    ...
    resp, r, err := cashfree.PGESReconVendor(&xApiVersion, &vendorReconRequest, &xRequestId, &xIdempotencyKey, &contentType, nil)
    if err != nil {
