...
func main() {
    ...
    orderCreateRefundRequest := *cashfree.NewOrderCreateRefundRequest(float64(123)) 
...
> RefundEntity PGOrderUpdateRefund(ctx, orderId, refundId).XApiVersion(xApiVersion).OrderUpdateRefundRequest(orderUpdateRefundRequest).XRequestId(xRequestId).XIdempotencyKey(xIdempotencyKey).Execute()
...
func main() {
    ...
    orderId := "your-order-id" 
    refundId := "some-refund-id" 
    orderUpdateRefundRequest := *cashfree.NewOrderUpdateRefundRequest("RefundStatus_example") 
    xRequestId := "4dfb9780-46fe-11ee-be56-0242ac120002" 
    xIdempotencyKey := "47bf8872-46fe-11ee-be56-0242ac120002" 

    resp, r, err := cashfree.PGOrderUpdateRefund(&xApiVersion, &orderId, &refundId, &orderUpdateRefundRequest, &xRequestId, &xIdempotencyKey, nil)
    if err != nil {
        fmt.Fprintf(os.Stderr, "Error when calling `cashfree.PGOrderUpdateRefund``: %v\n", err)
        fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
    }
    // response from `PGOrderUpdateRefund`: RefundEntity
    fmt.Fprintf(os.Stdout, "Response from `cashfree.PGOrderUpdateRefund`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiPGOrderUpdateRefundRequest struct via the builder pattern
